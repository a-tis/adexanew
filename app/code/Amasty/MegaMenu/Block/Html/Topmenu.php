        ...
        if ($node->hasChildren()) {
            ...
            foreach ($node->getChildren() as $childNode) {
                ...
                $arrowIcon = $childNode->hasChildren()
                    ...
                    : '';
                $html .= sprintf(
                    '<li class="ammenu-wrapper category-item %s" %s>%s
                    <a href="%s" title="%s" class="ammenu-link -level%s %s"><span class="custom-style ammenu-wrapper">%s</span><img class="custom-img" src="' . $this->getCategoryImage($childNode->getId()) . '"/> </a>',
