...
namespace Keepcalmlabs\HomePage\Block\Html;
...
class   Topmenu extends Template implements IdentityInterface, ArgumentInterface
{
    ...
    public function getCategoriesHtml(Node $node)
    {
        ...
        if (!isset($this->categoriesHtml[$id]) && $node->hasChildren()) {
            $this->categoriesHtml[$id] = '<div class="ammenu-categories-container ammenu-categories">'
        ...
        if ($node->hasChildren()) {
            ...
            foreach ($node->getChildren() as $childNode) {
                ...
                if ($level === 1
                    ...
                    //$html .= '<li class="ammenu-item -child' . $widthClass . '">';
                }

                $childNode->setPositionClass($itemPositionClassPrefix . $counter);
                ...
                $arrowIcon = $childNode->hasChildren()
                    ...
                    : '';
                //if(!$this->getCategoryImage($childNode->getId())){ .'style="width:auto"'. }
                $scheck = '';
                $border = '';
                if(!$this->getCategoryImage($childNode->getId())){
                    $scheck = 'style="width:auto;"';
                    $border = 'style="border: 1px solid transparent;height: auto !important;"';
                }
                $html .= sprintf(
                    '<li ' . $scheck . ' class="ammenu-wrapper category-item %s" %s>%s
                    <a href="%s" title="%s" class="ammenu-link -level%s %s"><span class="custom-style ">%s</span><span ' . $border .' class="custom-img"><img src="' . $this->getCategoryImage($childNode->getId()) . '"/></span> </a>',
                    $childNode->getPositionClass(),
                    $parentDataAttr,
                    $arrowIcon,
    ...
    private function isDirectivesExists($html)
    {
        ...
        return $matches || $this->moduleManager->isEnabled('Magento_PageBuilder');
    }
